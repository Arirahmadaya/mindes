...
const EditRealisasi = () => {
  ...
  useEffect(() => {
    const fetchRealisasiData = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/realisasi/${id_realisasi}`
        );
        const realisasiData = response.data.data[0];
        const formattedTglMulai = new Date(realisasiData.tgl_mulai)
          .toISOString()
          .split("T")[0];
        const formattedTglSelesai = new Date(realisasiData.tgl_selesai)
          .toISOString()
          .split("T")[0];

        setFormData({
          id_bidang: realisasiData.id_bidang,
          kode_kegiatan: realisasiData.kode_kegiatan,
          kegiatan: realisasiData.kegiatan,
          output: realisasiData.output,
          status: realisasiData.status,
          lokasi: realisasiData.lokasi,
          img_realisasi1: realisasiData.img_realisasi1,
          img_realisasi2: realisasiData.img_realisasi2,
          sumber: realisasiData.sumber,
          pembiayaan: realisasiData.pembiayaan,
          tgl_mulai: formattedTglMulai,
          tgl_selesai: formattedTglSelesai,
        });
        setSelectedBidangKey(new Set([realisasiData.id_bidang]));
        setSelectedStatus(new Set([realisasiData.status]));
        setSelectedKey(new Set([realisasiData.sumber]));
        setSelectedImage(
          `${import.meta.env.VITE_API_URL}${realisasiData.img_realisasi1}`
        );
        setSelectedImage1(
          `${import.meta.env.VITE_API_URL}${realisasiData.img_realisasi2}`
        );
      } catch (error) {
        console.error("Terjadi kesalahan saat mengambil data realisasi", error);
      }
    };
    ...
    <div className="flex flex-row w-screen h-screen overflow-y-auto bg-secondary-10">
      ...
      <div className="flex-1 mx-5">
        ...
        <Breadcrumbs className="my-5">
          ...
          <BreadcrumbItem href={`/admin/realisasi/edit/${id_realisasi}`}>
            Edit Realisasi
          </BreadcrumbItem>
        </Breadcrumbs>
